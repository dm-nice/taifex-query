...
def format_f03_output(date: str, status: str, data: Optional[Dict] = None, error: Optional[str] = None) -> str:
    ...
    if status == "success" and data:
        ...
        return f"F03: 台指期貨外資 [未平倉] [空方] : {short_pos:,} 口 [{source}]"
    else:
...
def extract_foreign_data_single(df: pd.DataFrame, date: str) -> Dict:
    ...
    if short_col is None:
        return {
            ...
            "date": date,
            "status": "failed",
            "error": f"找不到空方口數欄位，可用欄位: {df.columns.tolist()}"
        }
...
def fetch(date: str) -> str:
    ...
    try:
        ...
        return format_f03_output(date, "error", error="日期格式錯誤，請使用 YYYY-MM-DD")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return format_f03_output(date, "error", error=f"無法解析 HTML 表格: {str(e)}")

        if len(tables) == 0:
            return format_f03_output(date, "failed", error="該日無交易資料（可能是假日或休市日）")
        ...
        if result_dict.get("status") == "success":
            ...
            return format_f03_output(date, "failed", error=result_dict.get("error", "未知錯誤"))

    except requests.Timeout:
        return format_f03_output(date, "error", error="連線逾時，請檢查網路連線")

    except requests.HTTPError as e:
        return format_f03_output(date, "error", error=f"HTTP 錯誤 {e.response.status_code}")

    except requests.RequestException as e:
        return format_f03_output(date, "error", error=f"網路請求失敗: {str(e)}")

    except Exception as e:
        logger.exception("未預期的錯誤")
        return format_f03_output(date, "error", error=f"未預期的錯誤: {str(e)}")
