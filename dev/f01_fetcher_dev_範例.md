📄 2025.12.02 最新版 f01_fetcher_dev_範例.md（放在 dev/）
markdown
...
⚠️ 欄位名稱必須與 TAIFEX 網頁表頭一致，不可自行更改，例如：
...
- **必須支援**: 獨立執行測試
- **必須提供 fetch() 函式：即使模組尚未完成，也要有一個 stub，避免驗收流程中斷
...
✓ 成功時至少有 外資多方口數, 外資空方口數, 外資多空淨額

✓ 失敗時包含清晰的錯誤訊息

✓ 程式能在 Python 3.9 環境執行

✓ 必須包含 fetch(date: str) 函式，即使尚未完成，也需回傳失敗 JSON



## ✅ 驗收流程

1. 交付 `f01_fetcher_dev.py` 至 `dev/`
