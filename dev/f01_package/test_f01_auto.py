...
變更記錄：
- v2.0 (2025-12-07): 優化測試結果統計邏輯
  * 新增 evaluate_test_result() 函式，區分「實際狀態」與「測試結果」
  * 修正通過率計算：預期錯誤現在正確計為 PASS
  * 改進輸出：顯示測試結果 (PASS/FAIL) 和實際狀態 (success/error/failed/timeout)
  * 新增「測試結果統計」與「實際狀態分佈」兩個統計區塊
...
def validate_output_format(output: str) -> Dict:
    ...
    # 錯誤格式檢查
        ...
        checks['format_valid'] = (
            'F01 錯誤:' in output and
            'source: TAIFEX' in output
        )
