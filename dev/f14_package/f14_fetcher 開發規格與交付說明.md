...
模組代號: f14_fetcher 資料來源: 台灣期貨交易所 (TAIFEX) - 期貨每日交易行情 難度: ⭐⭐⭐☆☆ (3/5)

📋 專案目標
抓取 台指期貨 (TX) 當日收盤價 (Day N Close)，並統一輸出格式，方便後續分析與儀表板展示。

📂 專案結構與交付檔案
程式碼
f14_package/
├── README.md             # 通用開發指南（必讀）
├── f14_fetcher_spec.md   # 本規格文件
├── base.py               # 基礎類別定義
├── _template.py          # 開發範本
└── f14_fetcher_dev.py    # 您需交付的模組檔案
檔名: f14_fetcher_dev.py

MODULE 變數: "f14_fetcher_dev"

存放位置: 請直接存放在 f14_package/ 根目錄
...
挑戰：此網頁可能需要特定查詢參數（例如 queryDate=YYYY/MM/DD）或使用 POST 請求，請自行研究正確方式。
...
🛠️ 開發規範
1. 繼承與介面
必須繼承 base.py 中的 BaseFetcher 並實作 fetch 方法：
...
2. 錯誤處理
抓取失敗或無資料 → status="fail" 並在 error 欄位註明原因

禁止直接拋出 Exception，必須捕捉並回傳錯誤結果
...
3. 特殊處理邏輯
商品代號篩選：必須找到商品代號為「TX」或「臺股期貨」的行

到期月份選擇：選擇當月近月合約（例如 202512）

資料型別轉換：收盤價需去除千分位逗號並轉為 float
...
python
data = {"台指期貨收盤價": 27758.0}  # ✅ 正確
data = {"TX收盤價": 27758.0}       # ❌ 錯誤
未處理異常

必須使用 try-except 捕捉錯誤
...
使用瀏覽器 F12 → Network 觀察請求方式

📌 這份 f14_fetcher_spec.md 已經是最新完整版本，外包商只要看這一份就能完成 F14 模組。
